...
def replace(layers):
    ...
    for name, module in layers.named_children():
        if isinstance(module, torch.nn.Conv2d):
            # if 'conv2' in name:
            if cmp(module.kernel_size, (3, 3)) == 0:
                # i = i + 1
                # if (i != 3):
                #     continue
                new_module = DCN(module.in_channels, module.out_channels, module.kernel_size, module.stride,
                                 module.padding,
                                 module.dilation, 1)
...
def train_model(args, hyps):
    ...
    # print("asdfghjkl", model)
    replace_list = []
    for name, module in model.named_children():
        # print("aaaasd",module)
        if name == "backbone":
            for name1, module1 in module.named_children():
                if name1 not in replace_list:
                    continue
                replace(module1)
                # print(module1)
            break
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (ni, batch) in enumerate(pbar):
            ...
            s = ('%10s' * 2 + '%10.3g' * 5) % (
                '%g/%g' % (epoch, epochs - 1), '%.3gG' % mem, *mloss, mloss.sum(), gt_boxes.shape[1],
                min(ims.shape[2:]))
    ...
    # end training
    torch.dist.destroy_process_group() if torch.cuda.device_count() > 1 else None
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_path', type=str, default='/home/delta/home/E/MPD-master/HRSC2016/train.txt')
    parser.add_argument('--test_path', type=str, default='/home/delta/home/E/MPD-master/HRSC2016/test.txt')
